...
const exportExcel = async () => {
  ...
  try {
    ...
    worksheet['!cols'] = [
       { wch: 4 },  // 序号: index + 1,
       { wch: 12 },  // 用户ID: item.user_id,
       { wch: 20 },  // 创建时间: formatTime(item.timestamp),
       { wch: 20},  // 更新时间: formatTime(item.update_time),
       { wch: 10 },  // 嘉宾类型: item.guest_type,
       { wch: 8 },  // 桌号: item.table_num,
       { wch: 50 },  // 公司名称: item.company_name,
       { wch: 8 },  // 邀请码: item.invitation_code,
       { wch: 20 },  // 姓名: item.name,
       { wch: 15 },  // 性别: item.gender,
       { wch: 10 },  // 衣服尺码: item.cloth_size,
       { wch: 20 },  // 部门: item.deparment,
       { wch: 20 },  // 职位: item.job_title,
       { wch: 16 },  // 手机号: item.mobile_number,
       { wch: 20 },  // 邮箱: item.email,
       { wch: 10 },  // 房型: item.room_type,
       { wch: 12 },  // 酒店入住日期: item.checkin_date,
       { wch: 12 },  // 酒店退房日期: item.checkout_date,
       { wch: 12 },
       { wch: 12 },
       { wch: 12 },
       { wch: 12 },
       ...
       { wch: 10 },  // 抵达时间: formatTimeHM(item.arrival_hour, item.arrival_min),
       { wch: 15 },  // 是否需要接机: item.pickup_required,
       { wch: 12 },
       { wch: 12 },  // 返程日期: item.departure_date,
       { wch: 40 },  // 返程方式: item.departure_transport,
       { wch: 10 },  // 返程时间: formatTimeHM(item.departure_hour, item.departure_min),
       { wch: 15 },  // 是否需要送机: item.dropoff_required,
       { wch: 12 },
